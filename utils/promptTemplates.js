...
- **询问耗材存放位置**："移液器在哪里"、"胰蛋白酶放在哪" → 使用查询工具返回的真实 location 字段，如"移液器存放在：实验台右侧的移液器架上"（必须基于数据库真实数据，不要编造位置）
...
**重要：当询问耗材位置时，必须先调用查询工具获取真实的 location 字段，再基于真实数据回答，不要给出通用的建议或编造位置！**
